...
def main(
    ...
    spacy_model: str = typer.Option(
        ...
        "--spacy-model",
        "-s",
        help="Name of the spaCy model to use for lemmatization.",
    ),
):
    """
    Lemmatize all verbs in a text file to their infinitive form.
    """
    ...
    with output_path.open("w", encoding="utf-8") as f:
        for doc in tqdm(processed_lines, total=len(lines), desc="Processing lines"):
            ...
            for token in doc:
                if token.pos_ == "VERB":
                    modified_tokens.append(token.lemma_ + token.whitespace_)
                else:
                    modified_tokens.append(token.text_with_ws)
            f.write("".join(modified_tokens) + "\n")

    print(f"✅ Successfully saved lemmatized text to {output_path}")
